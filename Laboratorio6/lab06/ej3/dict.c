...
dict_t dict_remove(dict_t dict, key_t word) {
    ...
    if(dict != NULL) {
      ...
      // Estamos en la key a borrar, y el sub arbol derecho es nulo
      ...
      // Estamos en la key a borrar, y el sub arbol izquierdo es nulo
        ...
        struct _node_t ** substi; // Lugar donde esta el reemplazo

        // Busco el reemplazo,
        // elijiendo el minimo del subarbol de la derecha
        substi = &(dict->right);
        while((*substi)->left != NULL) {
          substi = &((*substi)->left);
        }
        
        aux = (*substi)->right; // Guardo el arbol derecho de mi reemplazo
        /* Asigno los subarboles del nodo por borrar al reemplazo */
        (*substi)->left = dict->left;
        (*substi)->right = dict->right;
        node_destroy(dict); // Destruyo el nodo a borrar
        dict = *substi; // Ubico el reemplazo
